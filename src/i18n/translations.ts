...
export const translations: Record<Language, Translations> = {
  zh: {
    ...
    header: {
      ...
      appSubtitle: '智能区分实质性篡改与 OCR 扫描噪声 · 一致率 ≥ 85% 自动放行',
    ...
    execute: {
      ready: '就绪，等待执行 Jev 独立状态评估与防篡改甄别',
      executing: '正在执行合同条款比对、甄别实质篡改与 OCR 识别噪声...',
    ...
    results: {
      ...
      lowConsistencyTitle: '需人工 Review（文本一致率低于 85%）',
      consistencyRate: '文本一致率',
      passedThreshold: '(达标 ≥ 85%)',
      belowThreshold: '(低于 85%)',
      ruleNotice: '规则: 无实质篡改且一致率 ≥ 85% 免审放行',
      tamperingCount: '实质篡改',
      ocrNoiseCount: '扫描噪点',
      copyVerdict: '复制结论',
      copied: '已复制',
  ...
  en: {
    ...
    header: {
      ...
      appSubtitle: 'Distinguish substantive tampering from OCR noise · Auto-pass when consistency ≥ 85%',
    ...
    execute: {
      ready: 'Ready. Waiting to execute Jev state evaluation and tampering detection',
    ...
    results: {
      ...
      lowConsistencyTitle: 'Manual Review Required (Consistency Below 85%)',
      consistencyRate: 'Text Consistency',
      passedThreshold: '(Meets ≥ 85%)',
      belowThreshold: '(Below 85%)',
      ruleNotice: 'Policy: Auto-pass when no tampering & consistency ≥ 85%',
    ...
    questionModal: {
      ...
      noulPromptPlaceholder: 'e.g. Does this clause have unauthorized alterations?',
      weightLabel: 'Weight',
